...
public class CommonFileUtils
{
  ...
  public static boolean copyFile(File in, File out)
     throws Exception
  {
    try (FileInputStream fis = new FileInputStream(in.getAbsolutePath()); FileOutputStream fos
     ...
     throws Exception
  {
    try (Reader r = encodingInput == null ? new InputStreamReader(new FileInputStream(in))
                       : new InputStreamReader(new FileInputStream(in), encodingInput); Writer w
     ...
     throws Exception
  {
    ...
    try (FileInputStream isOrig = new FileInputStream(fOrig); FileInputStream isDest = new FileInputStream(fDest))
    {
     ...
     throws Exception
  {
    ...
    try (FileInputStream fis = new FileInputStream(asciiFile); LineNumberReader br
   ...
   * Usa l'interfaccia serializzable per scrivere
   * il contenuto dell'oggetto.
   * Dopo la serializzazione i bytes sono compressi con GZIP.
   * @param toWrite oggetto da scrivere
   * @return array di bytes dell'oggetto
   * @throws Exception
   */
  public static byte[] writeObjectToBytesZipped(Serializable toWrite)
     throws Exception
  {
   ...
   * Usa l'interfaccia serializable per leggere
   * il contenuto dell'oggetto.
   * Prima della serializzazione i bytes sono decompressi usando GZIP.
   * @param array array di bytes con l'oggetto
   * @return oggetto serializzato
   * @throws Exception
   */
  public static Serializable readObjectFromBytesZipped(byte[] array)
     throws Exception
  {
    ByteBufferInputStream fis = new ByteBufferInputStream(false, array);
    ByteBufferOutputStream fos = new ByteBufferOutputStream();
    ...
    return readObjectFromBytes(fos.getBytes());
  }
     ...
     throws Exception
  {
    ...
    try (FileInputStream fis = new FileInputStream(asciiFileName); BufferedReader br
