...
{
  private ArrayDeque<FormattingOptions> optStack = new ArrayDeque<FormattingOptions>();

  public String format(FormattingOptions _opt) throws Exception
  {
    FormattingOptions last_opt = null;

    String result = "";
    if(!optStack.isEmpty())
    {
      int cnt = 0;
      Iterator<FormattingOptions> itr = optStack.descendingIterator();
      while(itr.hasNext())
      {
      ...
      last_opt = optStack.peekLast();
    }

    if(last_opt == null)
       last_opt = new FormattingOptions();

    FormattingOptions opt = (FormattingOptions) _opt.clone();
       ...
       && last_opt.chpVAlign != opt.chpVAlign)
    {
      ...
      last_opt = optStack.isEmpty() ? new FormattingOptions() : optStack.peekLast();
    }
  ...
  {
    String result = "";
    Iterator<FormattingOptions> itr = optStack.iterator();
    while(itr.hasNext())
    {
      itr.next();
      result += "</span>";
